CREATE OR REPLACE VIEW v_customer_service_ticket AS
SELECT
    t.ticket_id,
    c.complaint_id,
    ro.order_id,
...
CREATE OR REPLACE VIEW v_finance_complaint_ticket AS
SELECT
    t.ticket_id,
    ro.order_id,
    ct.type_name,
...
CREATE OR REPLACE VIEW v_safety_ticket AS
SELECT
    t.ticket_id,
    ro.order_id,
    ct.type_name,
...
CREATE OR REPLACE VIEW v_operation_ticket AS
SELECT
    t.ticket_id,
    ro.order_id,
    ct.type_name,
...
CREATE OR REPLACE VIEW v_manager_ticket_summary AS
SELECT
    d.department_name,
    ...
    SUM(CASE WHEN t.ticket_status <> '已关闭' AND t.sla_deadline < NOW() THEN 1 ELSE 0 END) AS overdue_tickets
FROM department d
LEFT JOIN ticket t ON d.department_id = t.department_id
GROUP BY d.department_id, d.department_name
ORDER BY d.department_name;

CREATE OR REPLACE VIEW v_employee_pending_ticket AS
SELECT
    t.ticket_id,
    t.current_owner_id,
    e.employee_name AS current_owner_name,
...
CREATE OR REPLACE VIEW v_feedback_result AS
SELECT
    f.feedback_id,
    f.ticket_id,
    ro.order_id,
